...
const KEYWORDS = ['with', 'type', 'private'];
const PRIMITIVE = [
	'int32',
	'int64',
	'float',
...
const name_mapping: Map<string, string> = new Map([
	...
	['int16', 'int32'],
	['int64', 'int32'],
	['T', 'string'] // unsafe (for dependent types)
]);
...
const transform = {
	...
	constant: (c: ConstantsType) =>
		`@val @module("${MODULE_META.name}") external ${cc2sc(c.name)}: ${format_type(c.type)} = "${
			c.name
		}"`,
...
const generate_enum = (enums: EnumType[]) => enums.map(transform.enum);
...
const generate_interface = () => {
	...
	TYPES.interfaces.map((i: InterfaceType) => {
		...
		i.properties.map((p) => {
			if (PRIMITIVE.includes(extract_type(p.type))) return;
	...
	// Patch in vanilla-data
	result.push('type number_range = { min: int, max: int }');
	return result;
};
...
const make_module = (m: Module, writer = new LayerString()): string => {
	...
	m.getter.map((g) => {
		const n = KEYWORDS.includes(g.name) ? `get${capitalize(g.name)}` : g.name;
...
const generate_type_map = (type_aliases: TypeFamily[]) =>
	type_aliases.map((t: TypeFamily) => {
...
const generate_errors = (errors: ErrorType[]) => errors.map(transform.error);
const generate_constants = (constants: ConstantsType[]) => constants.map(transform.constant);
const generate = (content: string) => {
	...
	const code = [
		generate_header(),
		tag('Enum definition'),
		generate_enum(TYPES.enums),
		tag('Error definition'),
		generate_errors(TYPES.errors),
		...
		generate_class(),
		tag('Type conversion'),
		generate_type_map(TYPES.type_aliases),
		tag('Constants'),
		generate_constants(TYPES.constants.concat(TYPES.objects))
	]
		.flat()
		.join('\n');
	return code;
