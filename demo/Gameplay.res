...
let init_break_the_terracotta = () => {
    ...
    switch score_obj.contents {
        | None => score_obj.contents = Some(world->scoreboard->addObjective("score", Some("level")))
        | Some(_) => ()
    }

    overworld->Dimension.getEntities(Some({exclude_types: ["player"]}))->map(Entity.kill)->ignore

    world->scoreboard->setObjectiveAtDisplaySlot(Sidebar, {objective: exn(score_obj.contents)})->ignore

    world->getAllPlayers->forEach((player: Player.t) => {
        open Player
        ...
        switch inv {
            ...
            | Some(inv) => {
                ...
                invComp->container->exn->Container.addItem(ItemStack.make(#String("diamond_sword"), 1))->ignore
